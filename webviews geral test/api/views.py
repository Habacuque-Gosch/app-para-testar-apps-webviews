...
from models import InfoColeta_entrega, Colet, Entreg
...
def calcula():
    ...
    #CHECK DE MONTAGEM
    montagem = request.form['check']
    if montagem == "":
        flash("Selecione se possui desmontagem ou não")
        return render_template('cotacao.html')
    
    # quantidae_montagem = request.form['quantidade']
    ...
    # Desmontagem e montagem: + R$140,00/móvel
    # Ajudante: + R$100,00/ajudante
    # Escadas: + R$5,00/andar/móvel
    # Embalagem: + R$12/caixa 

    if montagem == "sim":
        valor_corrida = valor_corrida + 140
